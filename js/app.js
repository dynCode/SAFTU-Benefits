...
module.controller('AppController', function($scope, $http, $window, $timeout, Upload) {
    $scope.apiPath = 'http://www.saftubenefits.co.za/api/';
    $scope.data = [];
    ...
    */
    ...
    $scope.init = function() {
        ...
        var didComp = $window.localStorage.getItem('didComp');
        
        $scope.didComp = didComp;
    ...
    $scope.registerME = function () {
        ...
        var unionCode = $scope.data.reg_unionCode;
        var shopCode = $scope.data.reg_shopCode;
        ...
        var saftuPayment = $scope.data.saftuPayment;
        var saftuEmp = $scope.data.saftuEmp;
        var saftuShop = $scope.data.saftuShop;
        var saftuShopHead = $scope.data.saftuShopHead;
        ...
        } else if (typeof unionCode === 'undefined' || unionCode === null || unionCode === '') {
             modal.hide();
            $scope.data.regBusy = false;
            ons.notification.alert({
                message: 'Please select your trade union to continue.',
                title: 'Oops!',
                buttonLabel: 'OK',
                animation: 'default'
            });
        } else {
            $http.post($scope.apiPath+'register.php', {"reqType" : "register", "unionCode" : unionCode, "shopCode" : shopCode, "signType" : signType, "FirstName" : FirstName, "LastName" : LastName, "gender" : gender, "title" : title, "IDNum" : IDNum, "dob" : dob, "CellNumber" : CellNumber, "EmailAddress" : EmailAddress, "Address1" : Address1, "Address2" : Address2, "PostCode" : PostCode, "Suburb" : Suburb, "City" : City, "Province" : Province, "saftuPayment" : saftuPayment, "saftuEmp" : saftuEmp, "saftuShop" : saftuShop, "saftuShopHead" : saftuShopHead, "Terms" : tc_y, "Privacy" : privacy_y, "Market" : market_y})
            .success(function(data, status){
                console.log("Data:", data);
                if (data['error'] == 0) {
                    console.log("Data:", data);
    ...
    */
    ...
    // check and save competition details
    $scope.enterComp = function() {
        $scope.data.regBusy = true;
        modal.show();
        $scope.data.errorCode = 'Processing, please wait...';
        
        var compq1 = $scope.data.compq1;
        var compq2 = $scope.data.compq2;
        var compq3 = $scope.data.compq3;
        var compq4 = $scope.data.compq4;
        var compq5 = $scope.data.compq5;
        var compq6 = $scope.data.compq6;
        var compCell = $scope.data.compCell;
        var compUnion = $scope.data.compUnion;
        
        // check all fields
        if (!compq1 && !compq2 && !compq3 && !compq4 && !compq5 && !compq6 && !compCell && !compUnion) {
            modal.hide();
            $scope.data.regBusy = false;
            ons.notification.alert({
                message: 'Please complete all the competition fields!',
                title: 'Oops!',
                buttonLabel: 'OK',
                animation: 'default'
            });
        } else {
            $http.post($scope.apiPath+'competition.php', {"reqType" : "enter", "compq1" : compq1, "compq2" : compq2, "compq3" : compq3, "compq4" : compq4, "compq5" : compq5, "compq6" : compq6, "compCell" : compCell, "compUnion" : compUnion, "mpacc" : $scope.userMpacc})
            .success(function(data, status){
                console.log("Data:", data);
                if (data['error'] == 0) {
                    console.log("Data:", data);
                    ...
                    ons.notification.alert({
                        ...
                        callback: function() {
                            $scope.didComp = true;
                            $window.localStorage.setItem('didComp',true);
                            $scope.data = [];
                            myNavigator.pushPage('views/home.html', { animation : 'fade' });
                        }
                    });
